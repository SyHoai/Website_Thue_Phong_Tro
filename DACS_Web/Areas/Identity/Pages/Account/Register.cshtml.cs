...
namespace DACS_Web.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(IFormFile filses, string selectedTinhThanh, string selectedQuanHuyen, string selectedPhuongXa,string returnUrl = null)
        {
            returnUrl ??= Url.Content("~/");
            ExternalLogins = (await _signInManager.GetExternalAuthenticationSchemesAsync()).ToList();
            if (ModelState.IsValid)
            {
                ApplicationUser userNameCheck = await _userManager.FindByNameAsync(Input.UserName);
                if (userNameCheck != null)
                {
                    ModelState.AddModelError("Input.UserName", "Tên đăng nhập đã tồn tại");
                    return Page();
                }
                ApplicationUser EmailCheck = await _userManager.FindByEmailAsync(Input.Email);
                if (EmailCheck != null)
                {
                ...
                {
                    ModelState.AddModelError("Input.Password", "Mật khẩu phải viết hoa chữ cái đầu");
                    ModelState.AddModelError("Input.Password", "Mật khẩu phải chứa 1 số và 1 ký tự");
                    return Page();
                }
                var user = CreateUser();
                if(filses == null)
                {
                    user.Avatar = "/images/avatar/AnhMacDinh.jpg";
                }
                else
                {
                ...
                if (result.Succeeded)
                {
                    if (!isAdmin)
                    {
                        ...
                        danhSach.TenDS = "YeuThich";
                        danhSach.User = user;
                        _context.DanhSach.Add(danhSach);
                    ...
                    if (isAdmin)
                    {
                        return RedirectToAction("QuanLiNhanVien", "User", new { area = "Admin" });
                    }
